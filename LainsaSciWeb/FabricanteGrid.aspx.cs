...
public partial class FabricanteGrid : System.Web.UI.Page
{
    ...
    protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            string strKey = item.GetDataKeyValue("FabricanteId").ToString();
            ...
            jCommand = String.Format("returnValues3('{0}','{1}','{2}');", strKey, item["Nombre"].Text, "Fabricante");
            ...
            jCommand = String.Format("editFabricante({0});", strKey);
            imgb.OnClientClick = jCommand;
            imgb.Visible = permiso.Ver;

            // assign to delete button (not needed by now)
            imgb = (ImageButton)item.FindControl("Delete");
            jCommand = String.Format("return radconfirm('{0}',event,300,100,'','{1}');", Resources.ResourceLainsaSci.DeleteRecordQuestion + " " + item["FabricanteId"].Text, Resources.ResourceLainsaSci.DeleteRecord);
            imgb.OnClientClick = jCommand;
    ...
    protected void RefreshGrid(bool rebind)
    {
        ...
        if(Usuario == null)
            RadGrid1.DataSource = CntLainsaSci.GetFabricantes(ctx);
        else
            RadGrid1.DataSource = CntLainsaSci.GetFabricantes(Usuario, ctx);
        if(rebind)
            RadGrid1.Rebind();
