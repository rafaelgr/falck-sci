...
function searchEstado(caller) {
    var oWnd = GetRadWindow();
    if (oWnd)
        oWnd.BrowserWindow.callEstadoGrid(caller);
    else
        alert("searchEstado(caller): no current window");
}
function searchPrioridad(caller) {
    var oWnd = GetRadWindow();
    if (oWnd)
        oWnd.BrowserWindow.callPrioridadGrid(caller);
    else
        alert("searchPrioridad(caller): no current window");
}
function searchResponsable(caller) {
    var oWnd = GetRadWindow();
    if (oWnd)
        oWnd.BrowserWindow.callResponsableGrid(caller);
    else
        alert("searchResponsable(caller): no current window");
}
