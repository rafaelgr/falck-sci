...
public partial class PrioridadGrid : System.Web.UI.Page
{
    ...
    protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            string strKey = item.GetDataKeyValue("PrioridadId").ToString();
            ...
            jCommand = String.Format("returnValues3('{0}','{1}','{2}');", strKey, item["Nombre"].Text, "Prioridad");
            ...
            jCommand = String.Format("editPrioridad({0});", strKey);
            imgb.OnClientClick = jCommand;
            imgb.Visible = permiso.Ver;

            // assign to delete button (not needed by now)
            imgb = (ImageButton)item.FindControl("Delete");
            jCommand = String.Format("return radconfirm('{0}',event,300,100,'','{1}');", Resources.ResourceLainsaSci.DeleteRecordQuestion + " " + item["PrioridadId"].Text, Resources.ResourceLainsaSci.DeleteRecord);
            imgb.OnClientClick = jCommand;
        ...
        if (typeOfControl.Equals("System.Web.UI.WebControls.ImageButton"))
        {
            ...
            switch (imgb.ID)
            {
                ...
                case "Delete":
                    try
                    {
                        Prioridad prioridad = CntLainsaSci.GetPrioridad(id, ctx);
                        CntLainsaSci.CTXEliminar(prioridad, ctx);
    ...
    {
        RadGrid1.DataSource = CntLainsaSci.GetPrioridads(usuario, ctx);
        if (rebind)
            RadGrid1.Rebind();
    }
    #endregion
