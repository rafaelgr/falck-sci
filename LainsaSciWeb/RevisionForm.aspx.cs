...
public partial class RevisionForm : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (revision != null)
            {
                ...
                if (revision.Dispositivo.Incidencias.Count > 0)
                {
                    string mensaje = String.Format("Hay {0} incidencias asociadas a esta revisión.\nPuede acceder a ellas haciendo clic en el botón inferior.",
                           revision.Dispositivo.Incidencias.Count);
                    RadWindowManager1.RadAlert(mensaje, 330, 110, "AVISO", "retornoAlert");
                }
            }
        }
    ...
    protected void btnAccept_Click(object sender, ImageClickEventArgs e)
    {
        ...
        try
        {
            if (!DataOk())
                return;
            if (revision.Estado != "REALIZADA") noRealizadPreviamente = true;
            if (newRecord)
                revision = new Revision();
            UnloadData(revision);
            CntLainsaSci.CTXGuardar(ctx);

            if (noRealizadPreviamente) // solo si el registro es nuevo se tocan planificaciones.
            {
    ...
    protected bool DataOk()
    {
            ...
            int id = int.Parse(control.ID);
                ...
                switch(tipo) {
                    case "ASP.camposn_ascx":
                        ...
                        if(csn.GetValor() == "" || csn.GetValor() == "|") {
                            err.Add(csn.GetNombre());
                        }
                        break;
                    case "ASP.campotext_ascx":
                        CampoText ctxt = (CampoText)control;
                        if(ctxt.GetValor() == "" || ctxt.GetValor() == "|") {
                            err.Add(ctxt.GetNombre());
                        }
                        break;
                    case "ASP.camponum_ascx":
                        CampoNum cnum = (CampoNum)control;
                        if(cnum.GetValor() == "" || cnum.GetValor() == "|") {
                            err.Add(cnum.GetNombre());

                        }
                        break;
                    case "ASP.campodate_ascx":
                        CampoDate cdt = (CampoDate)control;
                        if(cdt.GetValor() == "" || cdt.GetValor() == "|") {
                            err.Add(cdt.GetNombre());

                        }
                        break;
                    case "ASP.campotetra_ascx":
                        CampoTetra ctetra = (CampoTetra)control;
                        if(ctetra.GetValor() == "" || ctetra.GetValor() == "|") {
                            err.Add(ctetra.GetNombre());
                        }
                        break;
                }
        }
        if(err.Count > 0) {
            if(err.Count==1){
                RadNotification1.Text = "Falta rellenar 1 campo de la revisión.";
            }else{
                RadNotification1.Text = "Falta rellenar " + err.Count.ToString() + " campos de la revisión.";
            }
            RadNotification1.Show();
            return false;
        }
    ...
    protected void cargarPlantilla()
    {
        ...
        SortedList<int, string> datos_lst = new SortedList<int, string>();

        using(MySqlConnection connMy = new MySqlConnection(ConfigurationManager.ConnectionStrings["LainsaSciCTX"].ConnectionString)) {
            connMy.Open();
                ...
                cmdSrc.CommandText = @"
                    ...
                    FROM
	                    ...
	                    INNER JOIN (
		                    SELECT
			                    dr2.campo_id,
			                    MAX(dr2.datos_id) AS datos_id
		                    FROM
			                    revision r1
			                    INNER JOIN revision r2
				                    ON r1.dispositivo_id = r2.dispositivo_id
				                    AND r1.plantilla_id = r2.plantilla_id
			                    INNER JOIN datos_revision dr2
				                    ON dr2.revision_id = r2.revision_id
				                    AND dr2.valor <> '|'
		                    WHERE
			                    r1.revision_id = " + revision.RevisionId + @" 
		                    GROUP BY
			                    dr2.campo_id
	                    ) dr3
		                    ON dr1.datos_id = dr3.datos_id
                    ";
                using(MySqlDataReader dr = cmdSrc.ExecuteReader()) {
                    int campo_id = 0;
                    while(dr.Read()) {
                        campo_id = int.Parse(dr["campo_id"].ToString());
                        if(!datos_lst.ContainsKey(campo_id)) {
                            datos_lst.Add(campo_id, dr["valor"].ToString());
                        } else {
                            datos_lst[campo_id] = dr["valor"].ToString();
                        }
        ...
        foreach (DatosRevision item in revision.DatosRevisions.OrderBy(f => f.Campo.Posicion))
        {
            ...
            switch (tipo)
            {
                case "Campo OK/MAL":
                    ...
                    if(datos_lst.ContainsKey(item.Campo.CampoId) && (item.Valor=="|" ||item.Valor==null)) {
                        csn.SetValor(datos_lst[item.Campo.CampoId]);
                    } else {
                ...
                case "Texto":
                    ...
                    if(datos_lst.ContainsKey(item.Campo.CampoId) && (item.Valor == "|" || item.Valor == null)) {
                        ctxt.SetValor(datos_lst[item.Campo.CampoId]);
                    } else {
                ...
                case "Tetra":
                    ...
                    if(datos_lst.ContainsKey(item.Campo.CampoId) && (item.Valor == "|" || item.Valor == null)) {
                        ctt.SetValor(datos_lst[item.Campo.CampoId]);
                    } else {
                ...
                case "Numerico":
                    ...
                    if(datos_lst.ContainsKey(item.Campo.CampoId) && (item.Valor == "|" || item.Valor == null)) {
                        cnum.SetValor(datos_lst[item.Campo.CampoId]);
                    } else {
                ...
                case "Fecha":
                    ...
                    if(datos_lst.ContainsKey(item.Campo.CampoId) && (item.Valor == "|" || item.Valor == null)) {
                        cdt.SetValor(datos_lst[item.Campo.CampoId]);
                    } else {
        ...
        switch (tipo)
        {
            case "A":
                ...
                if (DateTime.TryParse(valor, out fechaBaja))
                {
                    ...
                    string mensaje = String.Format("El dispositivo ha sido dado de baja con fecha_apertura {0:dd/MM/yyyy}", fechaBaja);
                ...
                else
                {
                    ...
                    mensaje = "La fecha_apertura introducida es incorrecta";
                    RadWindowManager1.RadAlert(mensaje, 330, 110, "AVISO", "retornoAlert");
                    //string jCommand = String.Format("radalert('{0}');", mensaje);
                    //RadAjaxManager1.ResponseScripts.Add(jCommand);
                }
            ...
            case "B":
                if (valor == "true")
                {
                    ...
                    string url = String.Format("SustitucionForm.aspx?SustitucionId={0}", ns.SustitucionId);
                    string jCommand = String.Format("openOutSide('{0}','{1}');", url, "SustitucionForm");
                    RadAjaxManager1.ResponseScripts.Add(jCommand);
                }
                else
                {
    ...
    protected void btnIncidencia_Click(object sender, ImageClickEventArgs e)
    {
        string url = "";
        string jCommand = "";
        if (revision == null)
        ...
        {
            // este es el caso de que quiere hacer un cambio dentro de una revisión.
            url = String.Format("IncidenciaForm.aspx?DispositivoId={0}&RevisionId={1}", revision.Dispositivo.DispositivoId, revision.RevisionId);
            jCommand = String.Format("openOutSide('{0}','{1}');", url, "IncidenciaForm");
            RadAjaxManager1.ResponseScripts.Add(jCommand);
        }
        else
        {
            url = String.Format("IncidenciaGrid.aspx?DispositivoId={0}&RevisionId={1}", revision.Dispositivo.DispositivoId, revision.RevisionId);
            jCommand = String.Format("openOutSide('{0}','{1}');", url, "IncidenciaGrid");
            RadAjaxManager1.ResponseScripts.Add(jCommand);
        }
    }
