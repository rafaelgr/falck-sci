...
function openWindowFromMenu(url, name) {
    //alert("Estoy en openWindowFromMenu");
...
function openMenuItem(sender, args) {
    //alert("Estoy en openMenuItem");

    var itemValue = args.get_item().get_value();
    var tipo = itemValue.substring(0, 3);
    var url = "";
    if (tipo == "Rpt") {
        switch (itemValue) {
            case "RptInventario":
                url = "VisorInforme.aspx?Caller=MainMenu&Informe=" + itemValue;
                openWindowFromMenu(url, "VisorInforme");
                break;
            case "RptIncidenciaEmpresa":
                ...
                break;
            default:
                url = "VisorInformeCompuesto.aspx?Caller=MainMenu&Informe=" + itemValue;
                openWindowFromMenu(url, "VisorInformeCompuesto");
                break;
        }
    } else {
        switch (itemValue) {
...
function callRefreshGrid(caller, arg) {
    ...
    if (oWnd) {
        // it calls loadValues() inside the page, inside the window
       // alert(oWnd.get_contentFrame().contentWindow.refreshGrid);
...
function callEstadoGrid(caller) {
    var url = "EstadoGrid.aspx?Caller=" + caller + "&Mode=S";
    openWindow(url, "EstadoGrid");
}
function callPrioridadGrid(caller) {
    var url = "PrioridadGrid.aspx?Caller=" + caller + "&Mode=S";
    openWindow(url, "PrioridadGrid");
}
function callResponsableGrid(caller) {
    var url = "ResponsableGrid.aspx?Caller=" + caller + "&Mode=S";
    openWindow(url, "ResponsableGrid");
}
// --------------------------------------------------------------
...
function OnClientCommand(sender, args) {
  ...
  // if (args.get_commandName() == "Maximize") {
   //    alert("RadWindow Maximized");
     //  sender.InitialBehaviors = WindowBehaviors.Maximize;
    //}
}