...
public partial class ResumenRevisionGrid : System.Web.UI.Page
{
    ...
    protected void RadGrid1_ItemDataBound(object sender, GridItemEventArgs e)
    {
        ...
        if (e.Item is GridDataItem)
        {
            ...
            if(item["FechaUltima"].Text != "" || item["FechaSiguiente"].Text != "") {
                ImageButton img = (ImageButton)item.FindControl("Delete");
                if(img != null) {
                    img.Visible = false;
                }
            }
        ...
        if (typeOfControl.Equals("System.Web.UI.WebControls.ImageButton"))
        {
            ...
            switch (imgb.ID)
            {
                ...
                case "Delete":
                    try
                    {
                        ResumenRevision Resumen_Revision = CntLainsaSci.GetResumenRevision(id, ctx);
                        Resumen_Revision.estadoDispositivo = 1; // Estado no visible en pantalla
                        ctx.SaveChanges();
    ...
    protected void RefreshGrid(bool rebind)
    {
        ...
        /*else{
            foreach(ResumenRevision x in dispositivo.ResumenesRevisones) {
                if(x.estadoDispositivo == 1) {
                    dispositivo.ResumenesRevisones.Remove(x);
                }
            }}
             */
        if(dispositivo.ResumenesRevisones != null) {
            RadGrid1.DataSource = CntLainsaSci.GetResumenRevisions(dispositivo, ctx, 0);
            if(rebind)
                RadGrid1.Rebind();
        }
